...
from models.point_sem_seg import *
...
parser.add_argument('--model_path', default='log' + str(log_idx) + '/best_model/model.ckpt',
                    help='model checkpoint file path')
parser.add_argument('--dump_dir', default='log' + str(log_idx) + '/dump', help='dump folder path')
parser.add_argument('--output_filelist', default='log' + str(log_idx) + '/output_filelist.txt',
                    help='TXT filename, filelist, each line is an output for a room')
parser.add_argument('--room_data_filelist', default='meta/area' + str(log_idx) + '_data_label.txt',
...
def evaluate():
    ...
    for room_path in ROOM_PATH_LIST:
        room_path = room_path.replace('/home/stone/code/pointvlad-master/data',
                                      '/home/stone/code/Dataset/Pointdata/data/sem_seg')
...
def eval_one_epoch(sess, ops, room_path, out_data_label_filename, out_gt_label_filename):
    ...
    if FLAGS.visu:
        ...
        fout_img = open(os.path.join(DUMP_DIR, os.path.basename(room_path)[:-4] + '_img.obj'), 'w')
    ...
    for batch_idx in range(num_batches):
        ...
        feed_dict = {ops['pointclouds_pl']: current_data[start_idx:end_idx, :, :],  #### start_idx:end_idx, :, :
        ...
        for b in range(BATCH_SIZE):
            ...
            for i in range(NUM_POINT):
                ...
                if FLAGS.visu:
                    ...
                    fout_img.write('v %f %f %f %d %d %d\n' % (
                    pts[i, 6], pts[i, 7], pts[i, 8], int(pts[i, 3]), int(pts[i, 4]), int(pts[i, 5])))
